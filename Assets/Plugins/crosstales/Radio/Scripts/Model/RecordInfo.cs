      ...
      */
      ...
      private void parseStreamTitle()
      {
         ...
         if (startIndex != -1)
         {
            ...
            if (endIndex != -1)
            {
               streamTitle = part.Substring(0, endIndex).Replace('˗', '-').Replace('@', ' ').Replace('*', ' ').Replace('+', ' ').Replace(Constants.PATH_DELIMITER_WINDOWS, "-").Replace(Constants.PATH_DELIMITER_UNIX, "-").Trim();
