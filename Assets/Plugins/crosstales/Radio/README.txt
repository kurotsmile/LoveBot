# Radio PRO 2024.1.2

Thank you for buying our asset "Radio PRO"! 
If you have any questions about this asset, send us an email at [radio@crosstales.com](mailto:radio@crosstales.com). 
Please don't forget to rate it or even better write a little review – it would be very much appreciated.



## Description
Have you ever wanted to implement radio stations but don't want (or can't) pay an horrendous amount of money?
Whenever you like to provide good sound from famous artists for your games or apps, tune in on one of the 
uncountable Internet MP3 and OGG radio stations available for free.

Thanks to this asset, it is now possible for all Unity developers to listen to high quality sound 
without additional charges.
Alternatively receive music streams from your own server (e.g. Icecast, VLC Server etc.). 

We also implemented a unique sound visualizer in the demo scene, which you can modify to your liking.

Please read the "Radio-doc.pdf" and "Radio-api.pdf" for more details.



## Upgrade to new version
Follow this steps to upgrade the version of "Radio PRO":

1. Update "Radio PRO" to the latest version from the "Unity AssetStore"
2. Inside the project in Unity, go to menu "File" => "New Scene"
3. Delete the "Assets/Plugins/crosstales/Radio" folder from the Project-view
4. Import the latest version from the "Unity AssetStore"



## General note
We included the various stations in the demo scenes mainly to show that it works technically and 
also for fun (we like music and it was interesting to find all these different radio stations).
If you plan on including stations in a game that you release on a commercial basis, 
we strongly recommend that you contact the stations you want to use yourself.



## Release notes

See "VERSIONS.txt" for details.



## Credits

"Radio PRO" uses modified versions of:
* [NAudio 1.7.2](https://github.com/naudio/NAudio)
* [NLayer 1.1.0](https://github.com/naudio/NLayer)
* [NVorbis 0.8.4](https://github.com/NVorbis/NVorbis)
* [UDE](https://github.com/errepi/ude)

The included radio-stations are mainly provided by:
* [1.fm](https://www.1.fm)
* [SomaFM.com](https://www.somafm.com)
* [Energy](https://energy.ch)

The search is provided by:
* [Spotify](https://www.spotify.com)

The lyrics are provided by:
* [AZLyrics](https://www.azlyrics.com)

The querying and art APIs are provided by:
* [OnRadio](http://onrad.io)

We aren't affiliated to those companies.


The icons are based on [Font Awesome](https://fontawesome.io/).



## Contact

crosstales LLC
Schanzeneggstrasse 1
CH-8002 Zürich

* [Homepage](https://www.crosstales.com/en/portfolio/radio/)
* [Email](mailto:radio@crosstales.com)

### Social media
* [Discord](https://discord.gg/ZbZ2sh4)
* [Facebook](https://www.facebook.com/crosstales/)
* [Twitter](https://twitter.com/crosstales)
* [LinkedIN](https://www.linkedin.com/company/crosstales)



## More information
* [AssetStore](https://assetstore.unity.com/lists/crosstales-42213?aid=1011lNGT)
* [Forum](https://forum.unity.com/threads/radio-pro-mp3-and-ogg-streaming-solution.334604/)
* [Documentation](https://www.crosstales.com/media/data/assets/radio/Radio-doc.pdf)
* [API](https://www.crosstales.com/en/assets/radio/api)

### Videos
[Youtube-channel](https://www.youtube.com/c/Crosstales)

* [Promotion](https://youtu.be/1ZsxY788w-w?list=PLgtonIOr6Tb41XTMeeZ836tjHlKgOO84S)
* [Tutorial](https://youtu.be/E0s0NVRX-ec?list=PLgtonIOr6Tb41XTMeeZ836tjHlKgOO84S)

### Demos
* [Windows-Demo](https://drive.google.com/file/d/1uvfqDe2dWVGiVjaBqP2mwUvXgZUFIPQP/view?usp=sharing)
* [Mac-Demo](https://drive.google.com/file/d/1v0-KA2Xik0cat35destgOn8qwoe3xyKS/view?usp=sharing)
* [Linux-Demo](https://drive.google.com/file/d/1v5VMKx1VrobPtxzRmWuL14yGrpyVreyi/view?usp=sharing)
* [Android-Demo](https://drive.google.com/file/d/1vA4cTErl7N33djUpw9zeHBso4wR_0qvX/view?usp=sharing)

### Supported 3rd party assets
[All supported asset](https://assetstore.unity.com/lists/radio-friends-42211?aid=1011lNGT)

* [PlayMaker](https://assetstore.unity.com/packages/slug/368?aid=1011lNGT)
* [Audio Visualizer](https://assetstore.unity.com/packages/slug/47866?aid=1011lNGT)
* [Complete Sound Suite](https://assetstore.unity.com/packages/slug/19994?aid=1011lNGT)
* [Visualizer Studio](https://assetstore.unity.com/packages/slug/1761?aid=1011lNGT)
* [Apollo Visualizer Kit](https://assetstore.unity.com/packages/slug/59035?aid=1011lNGT)
* [Rhythm Visualizator](https://assetstore.unity.com/packages/slug/88041?aid=1011lNGT)
* [Volumetric Audio](https://assetstore.unity.com/packages/slug/17125?aid=1011lNGT)



`Version: 15.03.2024`