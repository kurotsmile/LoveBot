   ...
   class VorbisStreamDecoder : IVorbisStreamStatus, IDisposable
   {
      ...
      void ResetDecoder(bool isFullReset)
      {
         ...
         if (isFullReset)
         {
            ...
            //UnityEngine.Debug.Log("BLOCKSIZE: " + Block1Size);
      ...
      internal int ReadSamples(float[] buffer, int offset, int count)
      {
         ...
         lock (_seekLock)
         {
            if (_prevBuffer != null)
            {
               ...
               Array.Copy(_prevBuffer, 0, buffer, offset, cnt);
               ...
               if (cnt < _prevBuffer.Length)
               {
                  ...
                  Array.Copy(_prevBuffer, cnt * sizeof(float), buf, 0, (_prevBuffer.Length - cnt));
      ...
      {
         get { return TimeSpan.FromTicks(_sw.ElapsedTicks / PagesRead); }
      }

      public TimeSpan PacketLatency
      {
         get { return TimeSpan.FromTicks(_sw.ElapsedTicks / _packetCount); }
      }

      public TimeSpan SecondLatency
      {
         get { return TimeSpan.FromTicks((_sw.ElapsedTicks / _samples) * _sampleRate); }
      }

      public long OverheadBits
      {
         get { return _glueBits + _metaBits + _timeHdrBits + _wasteHdrBits + _wasteBits + _packetProvider.ContainerBits; }
      }

      public long AudioBits
      {
         get { return _bookBits + _floorHdrBits + _resHdrBits + _mapHdrBits + _modeHdrBits + _modeBits + _floorBits + _resBits; }
      }
