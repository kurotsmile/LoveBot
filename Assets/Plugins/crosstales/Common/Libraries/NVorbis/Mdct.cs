...
namespace Crosstales.NVorbis
{
   public class Mdct
   {
      ...
      public static void ClearSetupCache()
      {
         _setupCache.Clear();
      }
      ...
      private Mdct(int n)
      {
         ...
         for (k = k2 = 0; k < _n4; ++k, k2 += 2)
         {
            ...
            _B[k2] = (float)Math.Cos((k2 + 1) * M_PI / n / 2) * 0.5f;
            _B[k2 + 1] = (float)Math.Sin((k2 + 1) * M_PI / n / 2) * 0.5f;
         }
      ...
      readonly Dictionary<int, float[]> _threadLocalBuffers = new Dictionary<int, float[]>(1);
