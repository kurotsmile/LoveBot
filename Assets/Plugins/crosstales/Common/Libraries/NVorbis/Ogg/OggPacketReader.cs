...
namespace Crosstales.NVorbis.Ogg
{
   ...
   class PacketReader : IPacketProvider
   {
      class DebugView
      {
         ...
         public ContainerReader Container
         {
            get { return _reader._container; }
         }

         public int StreamSerial
         {
            get { return _reader._streamSerial; }
         }

         public bool EndOfStreamFound
         {
            get { return _reader._eosFound; }
         }
...
#pragma warning restore 67
      ...
      {
         if (!CanSeek) throw new InvalidOperationException();
         if (packetIndex < 0) throw new ArgumentOutOfRangeException("packetIndex");
         ...
         while (--packetIndex >= 0)
         {
            while (packet.Next == null)
            {
               if (_eosFound)
               {
                  throw new ArgumentOutOfRangeException("packetIndex");
               }
